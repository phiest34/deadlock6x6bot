...
Слой хранения вынесен за абстракцию репозитория: сейчас по умолчанию используется `sqlite`, но код подготовлен к замене backend-хранилища без переписывания бизнес-логики бота.
...
- при необходимости `STORAGE_BACKEND` (`sqlite` по умолчанию)
...
- добавить альтернативные backend-реализации хранилища помимо `sqlite`
